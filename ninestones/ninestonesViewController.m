...
#import "ninestonesViewController.h"

#define kNumberOfStones 9

@implementation ninestonesViewController
@synthesize stepsLabel;
@synthesize numberOfStonesInput;
...
- (void)viewDidLoad{
    ...
    [stepsLabel setText:[NSString stringWithFormat:@"%i stones can be split in %i steps.", kNumberOfStones, [self numberOfStepsForNumberOfStones:kNumberOfStones]]];
...
- (IBAction)updateUI{
    ...
    int numberOfStones = [numberOfStonesInput.text intValue];
    ...
    if (numberOfStones < 2) {
        [stepsLabel setText:@"You can't split that many stones."];
    }else{
        
        if (numberOfSteps == -1) {
            [stepsLabel setText:@"Whoa! That's a lot of stones!"];
        }else{
            
            [stepsLabel setText:[NSString stringWithFormat:@"%i stones can be split in %i %@.", numberOfStones, numberOfSteps, ((numberOfSteps != 1) ? @"steps" : @"step")]];
        }
    }
...
- (NSMutableArray *) stepsToSplitNStonesWhereNEquals:(int)numberOfStonesToSplit{
    ...
    NSMutableArray *stonesInEachStep = [[[NSMutableArray alloc] init] autorelease];
